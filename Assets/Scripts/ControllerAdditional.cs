...
public class ControllerAdditional : MonoBehaviour {

	private HandController handControl;
	private bool waitForClip;
	...
	private void DoButtonOnePressed(object sender, ControllerInteractionEventArgs e)
	{
		...
		if (e.controllerIndex == 1) {
			if (GameObject.Find ("Gun1").GetComponent<GunStatus> ().handGrabbed.Equals ("Right")) {
				match = true;
			}
		} else {
			if (GameObject.Find ("Gun1").GetComponent<GunStatus> ().handGrabbed.Equals ("Left")) {
				match = true;
			}
		}


		if (handControl.currentWeapon.Equals ("Gun1") && GameObject.Find ("Gun1").GetComponent<GunStatus> ().hasMag && match) {
			GameObject.Find ("Gun1").GetComponentInChildren<ReloadCheck> ().ClipRelease (1);

			GameObject.Find ("Gun1_Magazine").transform.SetParent (null);
			GameObject.Find ("Gun1_Magazine").AddComponent<Rigidbody> ();
			GameObject.Find ("Gun1").GetComponent<GunStatus> ().hasMag = false;

			handControl.audSource.PlayOneShot (handControl.pmClipOutSound);
			waitForClip = true;
		}
	}
	...
	void Update () {
		if (waitForClip) {
			...
			Debug.Log (clipTimer);
			if (clipTimer >= 0.15f) {
				GameObject.Find ("Gun1_Magazine").GetComponent<MeshCollider> ().enabled = true;
				GameObject.Find ("Gun1_Magazine").name = "Gun1_Magazine(Old)";
				waitForClip = false;
				clipTimer = 0;
			}
		}
